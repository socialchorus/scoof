describe("Scoof.TextUtils", function() {
  ...
  describe('shorten', function() {
    it('should shorten text that it is provided', function() {
      ...
      var shortened = Scoof.TextUtils.shorten(text, 10);
      expect(shortened).toBe('hello my n...');
    });

    it('fails gracefully with insufficient information', function() {
      expect(Scoof.TextUtils.shorten()).toBe(undefined);
    });
  });
  ...
  describe("stripTags", function () {
    it("should remove html tags", function() {
      ...
      expect(Scoof.TextUtils.stripTags(text)).toBe('Some text will be stripped of tags.');
    });
  });
});
